  ...
  <v-card>
    <v-card-text>
      ...
      <v-card
          ...
          height="100px"
          width="100px">
        <v-icon size="36" class="mt-7">mdi-calendar</v-icon>
      </v-card>

      <v-card-text class="text-right">
        <v-progress-circular v-show="loader.loading" :indeterminate="loader.loading" class="mr-3"/>
        <span class="grey--text">{{ $t('summary.schedules.title') }}</span>
        <h3> {{ schedules.itens.length }} </h3>
      </v-card-text>
    ...
    <v-card-actions>
      ...
      <span class="card-updated-time">{{ $t('summary.schedules.footer') }}</span>
    </v-card-actions>
  </v-card>
</template>
