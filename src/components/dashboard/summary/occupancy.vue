  ...
  <v-card>
    <v-card-text>
      ...
      <v-card
          ...
          height="100px"
          width="100px">
        <v-icon size="36" class="mt-7">mdi-gauge</v-icon>
      </v-card>

      <v-card-text class="text-right">
        <v-progress-circular v-show="loader.loading" :indeterminate="loader.loading" class="mr-3"/>
        <span class="grey--text">{{ $t('summary.occupancy.title') }}</span>
        <h3 v-show="!loader.loading"> {{ value }}% </h3>
      </v-card-text>
    ...
    <v-card-actions>
      ...
      <span class="card-updated-time">{{ $t('summary.occupancy.footer') }}</span>
    </v-card-actions>
  </v-card>
</template>
...
export default {
    ...
    async getRooms() {
      Room.get({
        ...
        this.rooms.itens.forEach(room => {
          this.availableTimetable += room.timetables.length
        })
      })
    },
  ...
  computed: {
    value() {
      //Use the rule of three to calculate the occupacy rate
      let daysInMonth = moment().daysInMonth();
      //TODO calculate quantity by days*available hours, because some days are not working days
      return ((this.schedules.itens.length * 100) / (this.availableTimetable * daysInMonth)).toFixed(2)
    }
  },

  mounted() {
    this.getRooms().then(() => this.getSchedules())
  }
